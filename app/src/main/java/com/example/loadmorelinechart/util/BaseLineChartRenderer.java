...
public class BaseLineChartRenderer extends LineRadarRenderer {

    private int faultAlarmIndex = 0;//绘制故障告警发生的时间的点
    ...
    private float hViewLength = Utils.convertDpToPixel(100f);//vie长100dp
    private float vViewLength = Utils.convertDpToPixel(18f);//view高18dp
    private float viewRect = Utils.convertDpToPixel(4f);//矩形高低差


    public BaseLineChartRenderer(Context context, int faultAlarmIndex, LineDataProvider chart, ChartAnimator animator,
                                 ViewPortHandler viewPortHandler) {
        super(animator, viewPortHandler);
        this.mContext = context;
        this.faultAlarmIndex = faultAlarmIndex;
        mChart = chart;
    ...
    public BaseLineChartRenderer(LineDataProvider chart, ChartAnimator animator,
                                 ViewPortHandler viewPortHandler) {
        super(animator, viewPortHandler);
        this.faultAlarmIndex = -1;
        mChart = chart;
        mCirclePaintInner = new Paint(Paint.ANTI_ALIAS_FLAG);
        mCirclePaintInner.setStyle(Paint.Style.FILL);
        mCirclePaintInner.setColor(Color.WHITE);
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                if (j != faultAlarmIndex) {
                    continue;//不是要绘制的点，跳过，默认是全部绘制点
                } else {
                    //需要绘制的点
                }
                /**********************绘制特定点 end********************************/
